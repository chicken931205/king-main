...
*/
	...
	function king_buy_post($oldquestion, $userid, $handle, $cookieid)
/*
	Set the author (application level) of $oldquestion to $userid and also pass $handle and $cookieid
	of user. Updates points and reports events as appropriate.
*/
	{
		...
		$postid = $oldquestion['postid'];
		$seller = $oldquestion['userid'];
		$postFormat = $oldquestion['postformat'];
		switch ($postFormat) {
			case 'music':
				$credit = qa_opt('c_msc');
				break;
		...
		require_once QA_INCLUDE_DIR.'king-db/selects.php';
		$buy = king_spend_credit($credit);
		if($buy) {
			if ( qa_opt('rate_auth') && $seller ) {
				$cre = (INT)qa_db_usermeta_get($seller, 'credit');
				$rn = qa_opt('rate_number');
				$rate_number = ($rn / 100) * $credit;
				$total = $cre + $rate_number;
				qa_db_usermeta_set( $seller, 'credit', $total );
			}
			qa_db_query_sub('INSERT INTO ^buyposts (buyer, seller, postid, price, created) VALUES (#, #, #, #, NOW())',
			$userid, $seller, $postid, $credit
			);
			return true;
		} else {
			return false;
