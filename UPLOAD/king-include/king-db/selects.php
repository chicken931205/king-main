 ...
 */
{
	...
	$selectspec = array(
		'columns'   => array(
			...
			'categoryname' => '^categories.title', 'categorycolor' => '^categories.color', 'categoryicon' => '^categories.icon', 'categorybackpath' => "^categories.backpath",
...
function qa_db_qs_selectspec_home($voteuserid, $sort, $ftime, $format, $filter, $start, $categoryslugs = null, $createip = null, $specialtype = false, $full = false, $count = null, $ifcount = null)
/*
Return the selectspec to retrieve questions (of type $specialtype if provided, or 'Q' by default) sorted by $sort,
restricted to $createip (if not null) and the category for $categoryslugs (if not null), with the corresponding vote
made by $voteuserid (if not null) and including $full content or not. Return $count (if null, a default is used)
 ...
 */
{
    ...
    switch ($sort) {
        ...
        case 'netvotes':
        case 'views':
            $sortsql = 'ORDER BY ^posts.' . $sort . ' ' . $ascending . ', ^posts.created DESC';
            break;

        case 'created':
        case 'hotness':
            $sortsql = 'ORDER BY ^posts.' . $sort . ' ' . $ascending;
            break;

        default:
            qa_fatal_error('qa_db_qs_selectspec_home() called with illegal sort value');
            break;
    }

    switch ($ftime) {
        case 'week':
            $timerange = 'AND DATE_SUB(CURDATE(), INTERVAL 7 DAY) <= created';
            break;

        case 'month':
            $timerange = 'AND DATE_SUB(CURDATE(), INTERVAL 30 DAY) <= created';
            break;

        case 'year':
            $timerange = 'AND DATE_SUB(CURDATE(), INTERVAL 1 YEAR) <= created';
            break;
        ...
        default:
            qa_fatal_error('qa_db_qs_selectspec_home() called with illegal time value');
            break;
    }
    ...
    if ($format) {
        ...
        $timerange .= ' AND postformat="' . qa_db_escape_string($format) . '"';
    }
    ...
    $selectspec['source'] .= " JOIN (SELECT postid FROM ^posts WHERE " .
        qa_db_categoryslugs_sql_args($categoryslugs, $selectspec['arguments']) .
        (isset($createip) ? "createip=INET_ATON($) AND " : "") .
        "type=$ " . $timerange . " " . $sortsql . " LIMIT #, #) y ON ^posts.postid=y.postid";
 ...
 */
{
	...
	$count = isset($count) ? $count : QA_DB_RETRIEVE_QS_AS;
...
function king_spend_credit($credit)
{
	require_once QA_INCLUDE_DIR . 'king-db/metas.php';
	$userid = qa_get_logged_in_userid();
	...
	if ($cre < $credit || $cre == 0 ) {
		...
		$total = $cre - $credit;
		qa_db_usermeta_set( $userid, 'credit', $total );
		return true;
	}

}

function king_check_bought($questionid, $userid, $author = null) {
	$query = qa_db_query_sub('SELECT * FROM ^buyposts WHERE buyer=# AND postid=# ', $userid, $questionid);
	$cont  = qa_db_read_one_assoc($query, true);
	if ($cont) {
		return true;
	} elseif(isset($author) && $author == $userid) {
		return true;
...
$postc ='';
if ( 'V' == $format ) {
	$postc = qa_lang_html( 'main/video' );
} elseif ( 'I' == $format ) {
	$postc      = qa_lang_html( 'main/image' );
} elseif ( 'N' == $format ) {
	$postc      = qa_lang_html( 'main/news' );
} elseif ( 'poll' == $format ) {
	$postc      = qa_lang_html( 'main/poll' );
} elseif ( 'list' == $format ) {
	$postc      = qa_lang_html( 'main/list' );
} elseif ( 'trivia' == $format ) {
	$postc      = qa_lang_html( 'main/trivia' );
} elseif ( 'music' == $format ) {
	$postc      = qa_lang_html( 'main/music' );
}
return $postc;
}
...
{
    $out = true;
    $usid = qa_get_logged_in_userid();
    
    if ( qa_opt('enable_adfree_mode') ) {            
        if ( qa_opt('adfree_logged') && qa_is_logged_in() ) {
            $out = false;
        } 
        
        if ( qa_opt('adfree_verify') && qa_get_logged_in_user_field('verified') ) {
            $out = false;
        }
        
        if ( qa_opt('adfree_points') ) {
            $user_point = qa_get_logged_in_user_field('points');
            if ( $user_point >= qa_opt('adfree_p') ) {
                $out = false;
            }
        }
        
        if ( qa_opt('adfree_pre') && qa_opt('enable_membership') && chekmembership($usid) ) {
            $out = false;
        }
    }
