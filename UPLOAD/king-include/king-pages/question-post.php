...
//	Process edit or save button for question
	...
	if ($question['editbutton'] || $question['retagcatbutton']) {
		if (qa_clicked('q_doedit')) {
			qa_redirect('edit', array('post' => $questionid));
...
*/
	...
	if ($question['postformat'] == 'I') {
		$imagesedit = preg_replace('(https*:\/\/[-\[\]\{\}\(\)\%_\/.a-zA-Z0-9+]+\.(png|jpg|jpeg|gif|bmp))', '<div class="submit-images"><img class="imagesedit" src="$0"/><input id="download" class="hide" value="$0" name="submit_image[]"><span class="king-remove"><i class="fas fa-trash"></i></span></div>', $edit);
	    $form['fields']['tiny']['html'].='<div id="multipleupload">Upload</div>'.$imagesedit.'';
	    $form['fields']['tiny']['label'].= qa_lang_html('main/image');
	} elseif ($question['postformat'] == 'V') {
		$form['fields']['tiny']['html'].='<input type="text" name="q_extra" class="king-form-tall-text" value="'.$edit.'">';
		$form['fields']['tiny']['label'].= qa_lang_html('main/video');
	} else {
