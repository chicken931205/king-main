...
if (qa_clicked('doask')) {
	...
	$submit_image = qa_post_array('submit_image'); // This will get the array of submit_image values
	$news_thumb = qa_post_text('news_thumb'); // This will get the value of news_thumb

	if (!empty($submit_image)) {
		$thumb = $submit_image[0];
	} else {
		$thumb = $news_thumb;
	}
...
$qa_content['title'] = qa_lang_html(isset($followanswer) ? 'question/ask_follow_title' : 'main/news');
